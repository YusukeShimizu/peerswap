...
func (p *PeerswapServer) SwapOut(ctx context.Context, request *SwapOutRequest) (*SwapResponse, error) {
	...
	swapOut, err := p.swaps.SwapOut(peerId, request.Asset, shortId.String(), pk, request.SwapAmount, request.AcceptablePremiumAmt)
	if err != nil {
		return nil, err
	}
...
func (p *PeerswapServer) SwapIn(ctx context.Context, request *SwapInRequest) (*SwapResponse, error) {
	...
	swapIn, err := p.swaps.SwapIn(peerId, request.Asset, shortId.String(), pk, request.SwapAmount, request.AcceptablePremiumAmt)
	if err != nil {
		return nil, err
	}
	// In order to be responsive we wait for the `opening_tx` to be sent before
