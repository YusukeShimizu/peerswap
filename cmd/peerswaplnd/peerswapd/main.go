...
func run() error {
	...
	if cfg.LiquidEnabled {
		if cfg.ElementsConfig.RpcUser != "" && cfg.LWKConfig.WalletName != "" {
		...
		if cfg.ElementsConfig.RpcUser != "" {
		...
		} else if cfg.LWKConfig.WalletName != "" {
			log.Infof("Liquid swaps enabled with LWK. Network: %s, wallet: %s", cfg.LWKConfig.Network, cfg.LWKConfig.WalletName)
			ec, err := electrum.NewClientTCP(ctx, cfg.LWKConfig.ElementsEndpoint)
			if err != nil {
				return err
			}
			// This call is blocking, waiting for elements to come alive and sync.
			liquidRpcWallet, err = wallet.NewLWKRpcWallet(client.NewLwk(cfg.LWKConfig.LWKEndpoint),
				ec, cfg.LWKConfig.WalletName, cfg.LWKConfig.SignerName)
			if err != nil {
				return err
			}
			...
			liquidOnChainService = onchain.NewLiquidOnChain(liquidRpcWallet, getLWKChain(cfg.LWKConfig.Network))
			supportedAssets = append(supportedAssets, "lbtc")
		} else {
