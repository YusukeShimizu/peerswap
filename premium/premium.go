...
// RemoveRate removes the premium rate for a given peer, asset, and operation.
func (p *Setting) RemoveRate(peerID string, asset AssetType, operation OperationType) error {
    err := p.store.RemoveRate(peerID, asset, operation)
    if err == nil {
        // Notify observers only if the removal was successful
        p.notifyObservers()
    }
    return err
}
